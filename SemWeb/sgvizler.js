    ...
    S.visualization = (function () {
        ...
        C.Text = charts.add(modSC, "Text",
            function (data, chartOpt) {
				...
				if (text == "Dead_Space" || text == "Crysis" || text == "Sim_City" || text == "Battlefield" || 
				    text == "FIFA" || text == "Need_for_Speed" || text == "Command_&_Conquer" || 
					text == "Burnout" || text == "NHL" || text =="Madden_NFL" || 
					text == "Medal_of_Honor" || text == "Dragon_Age" || text == "Mass_Effect" || 
					text == "The_Sims" || text == "NBA_Live") {
				var newText = "<a href='http://mjjmeijerink.github.io/SemWeb/" + text + ".html'>" + text + "</a>";
				text = newText;
				text = "<p>"+text+"</p>";
				}
                $(this.container)
         ...
         * Any number of columns. Everything is displayed as text.
         * 
         * @class sgvizler.visualization.Text2
         * @extends sgvizler.charts.Chart
         * @constructor
         * @param {Object} container The container element where the
         * chart will be drawn.
         ...
         * @since 0.3.0
         **/
        C.Text2 = charts.add(modSC, "Text",
            function (data, chartOpt) {
                var c, noColumns = data.getNumberOfColumns(),
                    r, noRows = data.getNumberOfRows(),
                    opt = $.extend({ cellSep: ', ',
                                     cellPrefix: '', cellPostfix: '',
                                     rowPrefix: '', rowPostfix: '</button>',
                ...
                for (r = 0; r < noRows; r += 1) {
                    row = opt.rowPrefix;
                    for (c = 0; c < noColumns; c += 1) {
						var x = "'" + data.getValue(r, c) + "'";
						x = x.replace(/\s/g, '');
						x = x.replace(/[):(]/g, '');
						row = '<button onclick="myFunction('+x+');">'
                        row += opt.cellPrefix + C.util.linkify2String(data.getValue(r, c)) + opt.cellPostfix;
                        if (c + 1 !== noColumns) { // Don't add for last element in row.
                            row += opt.cellSep;
                        }
